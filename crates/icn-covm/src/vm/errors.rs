...
impl From<StorageError> for VMError {
    fn from(err: StorageError) -> Self {
        match err {
            ...
            StorageError::IoError { operation, details } | StorageError::IOError { operation, details } => {
                VMError::StorageError {
                    details: format!("IO error during '{}': {}", operation, details)
                }
            }
            StorageError::TimeError { details } => {
                VMError::TimeError(details)
            }
            ...
            StorageError::VersionConflict { current, expected, resource } => {
                VMError::StorageError {
                    details: format!("Version conflict on '{}': current {}, expected {}", 
