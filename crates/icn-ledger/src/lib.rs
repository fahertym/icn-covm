...
impl DagLedger {
    ...
    pub fn nodes_by_namespace(&self, namespace: &str) -> Vec<&DagNode> {
        self.nodes.iter().filter(|n| n.namespace == namespace).collect()
    }
    ...
    fn trace_recursive(&self, node: &DagNode, result: &mut String, visited: &mut std::collections::HashSet<String>) -> Result<(), String> {
    ...
    fn export_recursive(&self, node_id: &str, result: &mut Vec<DagNode>, visited: &mut HashSet<String>) {
    ...
    pub fn export_selected_to_file(&self, start_ids: &[String], path: &Path) -> std::io::Result<usize> {
        // Create the directory if it doesn't exist
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)?;
        }
    ...
    pub fn diff_with(&self, other: &DagLedger) -> DagDiff {
        ...
        let common: Vec<String> = this_ids
    ...
    pub fn find_proposal_related_nodes(&self, proposal_id: &str) -> Vec<DagNode> {
        self.nodes.iter()
            .filter(|node| match &node.data {
                NodeData::ProposalCreated { proposal_id: id, .. } if id == proposal_id => true,
                NodeData::VoteCast { proposal_id: id, .. } if id == proposal_id => true,
                NodeData::ProposalExecuted { proposal_id: id, .. } if id == proposal_id => true,
                _ => false,
            })
            .cloned()
            .collect()
    ...
    pub fn get_namespaced_file_path(&self, namespace: &str) -> Result<String, String> {
        if let Some(file_path) = &self.file_path {
            ...
            let file_stem = path.file_stem().ok_or_else(|| "Invalid file path".to_string())?;
