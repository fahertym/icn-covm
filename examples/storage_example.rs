...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    storage.set_with_auth(&admin, "governance/members/admin", &serde_json::to_string(&admin_data)?)?;
    ...
    storage.set_with_auth(&admin, "governance/members/alice", &serde_json::to_string(&member1_data)?)?;
    ...
    storage.set_with_auth(&admin, "governance/members/bob", &serde_json::to_string(&member2_data)?)?;
    ...
    storage.set_with_auth(
        &member2, 
        "governance/delegations/bob/alice", 
        "1.0"
    )?;
    ...
    println!("  Votes cast and recorded. Current tally: {} for, {} against", 
    ...
    println!("  Admin account: {}/{} units remaining", 
        admin_account.balance, admin_account.quota);
    println!("  Alice account: {}/{} units remaining", 
        member1_account.balance, member1_account.quota);
    println!("  Bob account: {}/{} units remaining", 
    ...
    for (i, event) in storage.get_audit_log().iter().enumerate().take(10) {
        match event {
            StorageEvent::Access { key, action, user, timestamp } => {
                println!("  {}. [{}] User '{}' {}ed key '{}'", 
                    i+1, timestamp, user, action, key);
            },
            StorageEvent::Transaction { action, user, timestamp } => {
                println!("  {}. [{}] User '{}' performed transaction '{}'", 
                    i+1, timestamp, user, action);
            },
            StorageEvent::ResourceUsage { account, amount, operation, timestamp } => {
                println!("  {}. [{}] Account '{}' used {} units for '{}'", 
                    i+1, timestamp, account, amount, operation);
            }
        }
    }
