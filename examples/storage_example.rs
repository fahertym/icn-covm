...
use icn_covm::storage::{
    ...
    StorageError, VersionInfo, VersionDiff
};
...
fn main() -> StorageResult<()> { // Use StorageResult for consistency
    ...
    println!("\n--- Basic Operations ---");
    storage.set(Some(&admin), namespace, "config/site_name", "My Coop Platform".as_bytes().to_vec())?;
    ...
    storage.set_json(Some(&admin), namespace, "governance/proposals/prop-001", &proposal)?;
    
    let retrieved_prop: Proposal = storage.get_json(Some(&admin), namespace, "governance/proposals/prop-001")?;
    ...
    storage.set_json(Some(&member1), namespace, "governance/proposals/prop-001", &updated_prop)?;
    
    // Update again (implicitly creates version 3)
    updated_prop.status = "closed".to_string();
    updated_prop.votes.insert("alice".to_string(), "for".to_string());
    storage.set_json(Some(&member1), namespace, "governance/proposals/prop-001", &updated_prop)?;
    ...
    match storage.list_versions(Some(&admin), namespace, "governance/proposals/prop-001") {
        Ok(versions) => {
            for version in versions {
                println!("  - Version: {}, Created By: {}, Timestamp: {}",
                    version.version, version.created_by, version.timestamp); // Use created_by
            }
        }
    ...
    let (version1_bytes, _) = storage.get_version(Some(&admin), namespace, "governance/proposals/prop-001", 1)?;
    let version1: Proposal = serde_json::from_slice(&version1_bytes).map_err(|e| StorageError::SerializationError { details: e.to_string() })?;
    ...
    let latest_version: Proposal = serde_json::from_slice(&latest_bytes).map_err(|e| StorageError::SerializationError { details: e.to_string() })?;
    ...
    storage.set(Some(&member1), namespace, "governance/votes/prop-001/alice", b"for".to_vec())?;
    storage.set(Some(&member2), namespace, "governance/votes/prop-001/bob", b"delegated:alice".to_vec())?;
    
    // Retrieve audit log
    match storage.get_audit_log(Some(&admin), Some(namespace), None, 10) { // Pass auth, namespace
        Ok(events) => {
            println!("Last {} audit events:", events.len());
            for (i, event) in events.iter().enumerate() {
                print!("  {}: ", i + 1);
                match event {
                    icn_covm::storage::StorageEvent::Set { key, user, timestamp, .. } => {
                        println!("Set key '{}' by '{}' at {}", key, user.as_deref().unwrap_or("unknown"), timestamp);
                    }
                    icn_covm::storage::StorageEvent::Delete { key, user, timestamp, .. } => {
                        println!("Delete key '{}' by '{}' at {}", key, user.as_deref().unwrap_or("unknown"), timestamp);
                    }
                    icn_covm::storage::StorageEvent::Transaction { action, user, timestamp, .. } => {
                        println!("Transaction '{}' by '{}' at {}", action, user.as_deref().unwrap_or("unknown"), timestamp);
                    }
    ...
    if let Ok(vote_keys) = vote_keys_result {
        println!("Found {} votes/delegations for proposal {}", vote_keys.len(), final_prop.id);
        for key in vote_keys {
            if let Ok(vote_bytes) = storage.get(Some(&admin), namespace, &key) { // Pass auth, namespace
    ...
    println!("Vote Results for '{}': Yes: {}, No: {}, Total Direct: {}", final_prop.title, yes_votes, no_votes, total_votes);
