...
use icn_covm::storage::{
    AuthContext, InMemoryStorage, ResourceAccount, StorageBackend, StorageEvent, StorageResult, StorageError, VersionInfo
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let vote2 = Vote { voter: member2_id.to_string(), choice: "against".to_string(), reason: Some("Budget is fine".to_string()), timestamp: icn_covm::storage::now() };
    let vote_result = storage.set_json_with_resources(Some(&member2_auth), namespace, "votes/prop-001/member_2", &vote2);
    ...
    let mut prop_data: ProposalData = storage.get_json(Some(&admin_auth), namespace, "proposals/prop-001")?;
    
    // (Simplified tally - assumes direct votes exist as keys)
    let votes_for = storage.contains(Some(&admin_auth), namespace, "votes/prop-001/member_1")?.then_some(1).unwrap_or(0);
    let votes_against = storage.contains(Some(&admin_auth), namespace, "votes/prop-001/member_2")?.then_some(1).unwrap_or(0);
    ...
    storage.set_json_with_resources(Some(&admin_auth), namespace, "proposals/prop-001", &prop_data)?;
    
    storage.set(Some(&admin_auth), namespace, "config/last_proposal_result", prop_data.status.as_bytes().to_vec())?;
            ...
            for event in events {
                match event {
                    ...
                    icn_covm::storage::StorageEvent::Delete { key, user, .. } => {
                        println!("  Delete '{}' by {}", key, user.as_deref().unwrap_or("anon"));
                    }
                    icn_covm::storage::StorageEvent::Transaction { action, user, timestamp, .. } => {
                        println!("  Tx '{}' by {} @ {}", action, user.as_deref().unwrap_or("anon"), timestamp);
                    }
