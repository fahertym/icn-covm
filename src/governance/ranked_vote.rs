...
impl GovernanceOpHandler for RankedVoteHandler {
    ...
    {
        if let Op::RankedVote { candidates, ballots } = op {
            // Validate parameters
            if *candidates < 2 {
            ...
            vm.executor.emit_event("governance", &format!("Running ranked-choice vote with {} candidates and {} ballots", candidates, ballots));
            ...
            while remaining_candidates > 1 {
                ...
                vm.executor.emit_event("governance", &format!("Eliminated candidate {} with {} votes", min_candidate, min_votes));
            }
            
            // Find the winner (last non-eliminated candidate)
            let winner = eliminated.iter().position(|&e| !e).unwrap_or(0);
            
            vm.executor.emit_event("governance", &format!("Winner of ranked-choice vote: candidate {}", winner));
            
            // Push the winner to the stack
            vm.stack.push(winner as f64);
            Ok(())
        } else {
            Err(VMError::UndefinedOperation("Expected RankedVote operation".into()))
        }
    }
} 