...
impl FileStorage {
    ...
    fn version_path(&self, namespace: &str, key: &str, version: u64) -> PathBuf {
        self.key_dir_path(namespace, key)
            .join(format!("v{}.data", version))
    }
    ...
    fn read_key_metadata(&self, namespace: &str, key: &str) -> StorageResult<KeyMetadata> {
        ...
        let file = File::open(&path)
            .map_err(|e| self.map_io_error(e, namespace, Some(key), "opening metadata file"))?;
        ...
        let metadata_str = fs::read_to_string(path)
            .map_err(|e| self.map_io_error(e, namespace, Some(key), "reading metadata file"))?;
        ...
        if let Some(parent) = path.parent() {
            create_dir_all(parent).map_err(|e| {
                self.map_io_error(
                    ...
                    namespace,
                    Some(key),
                    format!("creating directory for version {}", version).as_str(),
                )
            })?;
        }
    ...
    fn write_namespace_metadata(&self, metadata: &NamespaceMetadata) -> StorageResult<()> {
        ...
        let file = OpenOptions::new()
            ...
            .map_err(|e| {
                self.map_io_error(
                    ...
                    &metadata.path,
                    None,
                    "opening namespace metadata file for writing",
                )
            })?;
        ...
        file.lock_exclusive().map_err(|e| {
            self.map_io_error(
                ...
                &metadata.path,
                None,
                "locking namespace metadata file for writing",
            )
        })?;
    ...
    fn record_audit_log(
        ...
        file.lock_exclusive()
            .map_err(|e| self.map_io_error(e, namespace, key, "locking audit log file"))?;

        // Write the log entry
        writeln!(file, "{}", log_str)
            .map_err(|e| self.map_io_error(e, namespace, key, "writing to audit log"))?;
            ...
            std::io::ErrorKind::NotFound => {
                if let Some(k) = key {
                    StorageError::NotFound {
                        key: format!(
                            "{}:{} (IO error: file not found during {})",
                            namespace, k, operation
                        ),
                    }
                } else {
                    StorageError::NotFound {
...
impl StorageBackend for FileStorage {
    fn get(
        ...
        self.record_audit_log(
            ...
            namespace,
            Some(key),
            &format!("Read version {}", latest_version.version),
        )?;
    ...
    fn set(
        ...
        let version_info = if key_metadata_exists {
            ...
            let latest_version = metadata.versions.last().map(|v| v.version).unwrap_or(0);
    ...
    fn rollback_transaction(&mut self) -> StorageResult<()> {
        ...
        for op in transaction.into_iter().rev() {
            match op {
                    ...
                    match (previous_data, prev_version) {
                        (Some(data), Some(version)) => {
                            ...
                            while metadata
                                .versions
                                .last()
                                .map_or(false, |v| v.version > version.version)
                            {
                ...
                TransactionOp::DeleteNamespace {
                    namespace,
                    metadata,
                } => {
                    // Restore the deleted namespace
                    self.namespace_cache
                        .insert(namespace.clone(), metadata.clone());
    ...
    fn create_namespace(
        ...
        let can_create = auth.map_or(false, |a| {
        ...
        let namespace_dir = self.namespace_path(namespace);
        create_dir_all(&namespace_dir)
            .map_err(|e| self.map_io_error(e, namespace, None, "creating namespace directory"))?;
    ...
    fn list_namespaces(
    ...
    ) -> StorageResult<Vec<crate::storage::namespaces::NamespaceMetadata>> {
        ...
        for (path, metadata) in &self.namespace_cache {
            ...
            if auth.map_or(false, |a| {
                a.has_role(path, "reader")
