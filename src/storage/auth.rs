...
impl AuthContext {
    ...
    pub fn add_role_to_identity(&mut self, identity_did: &str, namespace: &str, role: &str) {
        let namespace_roles = self.roles.entry(namespace.to_string()).or_insert_with(HashMap::new);
        let role_identities = namespace_roles.entry(role.to_string()).or_insert_with(HashSet::new);
    ...
    pub fn add_delegation(&mut self, delegator_did: &str, delegate_did: &str, delegation_type: &str) {
    ...
    pub fn is_member(&self, identity_did: &str, namespace: &str) -> bool {
        self.memberships.iter().any(|m| m.identity_did == identity_did && m.namespace == namespace)
    }
    ...
    pub fn has_delegation(&self, delegator_did: &str, delegate_did: &str) -> bool {
        self.delegations.iter().any(|d| 
            d.delegator_did == delegator_did && 
            d.delegate_did == delegate_did
        )
    }
