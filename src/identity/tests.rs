...
mod tests {
    ...
    fn test_identity_metadata() {
        ...
        assert_eq!(
            identity.get_metadata("email"),
            Some(&"charlie@example.com".to_string())
        );
        assert_eq!(
            identity.get_metadata("coop_id"),
            Some(&"coop123".to_string())
        );
    ...
    fn test_member_profile() {
        ...
        assert_eq!(
            profile.attributes.get("bio"),
            Some(&"Cooperative developer".to_string())
        );
    }
    ...
    fn test_credential() {
        let timestamp = now();
        let mut credential =
            Credential::new("cred123", "membership", "coop123", "alice", timestamp);
    ...
    fn test_delegation_link() {
        let timestamp = now();
        let mut delegation = DelegationLink::new("del123", "alice", "bob", "voting", timestamp);
        ...
        assert_eq!(
            delegation.attributes.get("context"),
            Some(&"annual_meeting".to_string())
        );
    }
}
