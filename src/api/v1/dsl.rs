...
use crate::storage::{Storage, StorageExtensions};
use crate::vm::VM;
...
pub fn get_routes(
    ...
    let create_macro = base
        ...
        .and(with_auth())
        .and(require_role("dsl:write".to_string()))
    ...
    let execute_macro = base
        ...
        .and(with_vm(vm.clone()))
        .and(with_auth())
        .and(require_role("dsl:execute".to_string()))
...
fn with_vm(vm: Arc<VM>) -> impl Filter<Extract = (Arc<VM>,), Error = std::convert::Infallible> + Clone {
...
async fn list_macros_handler(
    ...
    let macros = storage.list_macros().await
        .map_err(|e| internal_error(e.to_string()))?;
    
    // Apply sorting if specified
    let mut macro_list = macros;
        ...
        match sort_by.as_str() {
            "name" => {
                if is_ascending {
                    ...
                    macro_list.sort_by(|a, b| b.name.cmp(&a.name));
                }
            },
            "updated_at" => {
                if is_ascending {
                    macro_list.sort_by(|a, b| a.updated_at.cmp(&b.updated_at));
                } else {
                    macro_list.sort_by(|a, b| b.updated_at.cmp(&a.updated_at));
                }
            },
    ...
    // Apply pagination
    let total = macro_list.len();
    let start = (page - 1) * page_size;
    let end = std::cmp::min(start + page_size, total);
    
    let paged_macros = if start < total {
        macro_list[start..end].to_vec()
    } else {
        vec![]
    };
    
    // Convert to API model
    let summaries: Vec<MacroSummary> = paged_macros.into_iter()
        .map(|m| MacroSummary {
            id: m.id,
            name: m.name,
...
async fn create_macro_handler(
    ...
    let macro_def = crate::storage::MacroDefinition {
        ...
        created_by: Some(auth.user_id.unwrap_or_else(|| "anonymous".to_string())),
