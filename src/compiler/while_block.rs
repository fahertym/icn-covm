...
pub fn parse_while_block(lines: &[String], current_line: &mut usize, pos: SourcePosition) -> Result<Op, CompilerError> {
        ...
        if line.trim() == "condition:" {
            ...
            *current_line += 1;
            ...
            if line.trim() == "if:" {
                ...
                let nested_op = super::loop_block::parse_loop_block(lines, current_line, nested_pos)?;
                body.push(nested_op);
            } else if line.trim() == "match:" {
                let nested_op = super::match_block::parse_match_block(lines, current_line, nested_pos)?;
                body.push(nested_op);
            } else {
                return Err(CompilerError::UnknownBlockType(line.trim().to_string(), nested_pos.line, nested_pos.column));
            }
        } else {
    ...
    fn test_nested_while_blocks() {
        ...
        match op {
            Op::While { condition, body } => {
                ...
                // Check nested while
                match &body[0] {
                    Op::While { condition: nested_condition, body: nested_body } => {
