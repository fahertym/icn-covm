use crate::vm::Op;
use super::{CompilerError, SourcePosition, common};
...
pub fn parse_line(line: &str, pos: SourcePosition) -> Result<Op, CompilerError> {
    ...
    match command {
        "push" => {
            let num_str = parts.next().ok_or(CompilerError::MissingPushValue(pos.line, pos.column))?;
            let num = num_str.parse::<f64>()
                .map_err(|_| CompilerError::InvalidPushValue(
                    num_str.to_string(), 
                    pos.line, 
        ...
        "assertequalstack" => {
            let depth_str = parts.next().ok_or(CompilerError::MissingAssertDepth(pos.line, pos.column))?;
            let depth = depth_str.parse::<usize>()
                .map_err(|_| CompilerError::InvalidAssertDepth(
                    depth_str.to_string(), 
                    pos.line, 
        ...
        "return" => Ok(Op::Return),
        "call" => {
            let fn_name = parts.next().ok_or(CompilerError::MissingFunctionName(pos.line, pos.column))?;
        ...
        _ => Err(CompilerError::UnknownCommand(command.to_string(), pos.line, pos.column)),
    }
}

/// Parse a series of lines as a block of code
pub fn parse_block(lines: &[String], start_line: &mut usize, base_indent: usize, pos: SourcePosition) -> Result<Vec<Op>, CompilerError> {
    ...
    while *start_line < lines.len() {
        ...
        if line.trim().ends_with(':') {
            let op = if line.trim() == "if:" {
                ...
                return Err(CompilerError::UnknownBlockType(line.trim().to_string(), current_pos.line, current_pos.column));
            };
