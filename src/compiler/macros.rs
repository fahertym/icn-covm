...
fn parse_proposal_block(lines: &[&str]) -> Result<(HashMap<String, String>, HashMap<String, String>, String), String> {
...
pub fn expand_proposal_lifecycle(lines: &[&str]) -> Result<Vec<Op>, String> {
    ...
    let creator = "macro_creator".to_string(); // Placeholder
    let proposal = ProposalLifecycle::new(
        proposal_id, creator, title.clone(), quorum, threshold, discussion_duration, None,
    );
    ...
    let mut ops = vec![
        Op::Emit(format!("Creating proposal {}...", proposal_id)),
    ];
    ...
    for (name, file_path_str) in attachments {
        ...
        let store_attachment_op_str = format!("StoreP \"{}\"/\"{}\" \"{}\"", namespace, attachment_key, escaped_content);
        // Map CompilerError to String
    ...
    if !execution_dsl.is_empty() {
        ...
        println!("[MACRO] Storing execution DSL ({} bytes) to key: {}/{}...", execution_dsl.len(), namespace, logic_attachment_key);
        let escaped_dsl = execution_dsl.replace("\\", "\\\\").replace("\"", "\\\"");
        let store_logic_op_str = format!("StoreP \"{}\"/\"{}\" \"{}\"", namespace, logic_attachment_key, escaped_dsl);
        // Map CompilerError to String
        ...
        ops.push(Op::EmitEvent {
         ...
         println!("[MACRO] No execution block found for proposal {}.", proposal_id);
    }
