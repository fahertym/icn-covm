    ...
    while *current_line < lines.len() {
        ...
        if line.trim() == "value:" {
            ...
            *current_line += 1;
            ...
            // Convert to TypedValue::Number for the typed-values feature
            let typed_value = crate::typed::TypedValue::Number(case_value);
            cases.push((typed_value, case_ops));
        } else if line.trim() == "default:" {
            *current_line += 1;
            let default_indent = indent;
    ...
    fn test_match_block_parsing() {
        ...
        match op {
            Op::Match { value, cases, default } => {
                ...
                // Check case values
                assert_eq!(cases[0].0, crate::typed::TypedValue::Number(1.0));
                assert_eq!(cases[1].0, crate::typed::TypedValue::Number(2.0));
