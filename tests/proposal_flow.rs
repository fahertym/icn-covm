...
fn test_proposal_quorum_failure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bob_auth = create_user_auth("bob");
    vm.auth_context = Some(bob_auth.clone());
    let vote_matches_bob = cli_app.get_matches_from(vec!["proposal", "vote", "--id", &proposal_id, "--choice", "yes"]);
    handle_proposal_command(vote_matches_bob.subcommand_matches("proposal").unwrap(), &mut vm, &bob_auth)?;
    ...
    assert_eq!(final_proposal.state, ProposalState::Rejected, "Proposal should be Rejected due to unmet quorum");
...
fn test_proposal_threshold_failure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let create_matches = cli_app.get_matches_from(vec![
        ...
        "--quorum", "2", 
        "--threshold", "2" // Requires 2 Yes votes
    ]);
    handle_proposal_command(create_matches.subcommand_matches("proposal").unwrap(), &mut vm, &alice_auth)?;
    let proposal_id = find_proposal_id(&vm, &alice_auth)?;
    ...
    let charlie_auth = create_user_auth("charlie");
    vm.auth_context = Some(charlie_auth.clone());
    let vote_matches_charlie = cli_app.get_matches_from(vec!["proposal", "vote", "--id", &proposal_id, "--choice", "no"]);
    handle_proposal_command(vote_matches_charlie.subcommand_matches("proposal").unwrap(), &mut vm, &charlie_auth)?;

    // Check final state (should be Rejected)
    vm.auth_context = Some(alice_auth.clone());
    let final_proposal = load_proposal(&vm, &proposal_id)?;
    assert_eq!(final_proposal.state, ProposalState::Rejected, "Proposal should be Rejected due to unmet threshold");
...
fn test_proposal_execution_failure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    vm.auth_context = Some(alice_auth.clone());
    let final_proposal = load_proposal(&vm, &proposal_id)?;
    assert_eq!(final_proposal.state, ProposalState::Executed, "Proposal should be Executed despite logic failure");
    assert!(matches!(final_proposal.execution_status, Some(ExecutionStatus::Failure(_))), "Execution status should be Failure");
...
fn test_proposal_edit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let create_matches = cli_app.get_matches_from(vec!["proposal", "create", "--title", initial_title, "--quorum", "1", "--threshold", "1"]);
    handle_proposal_command(create_matches.subcommand_matches("proposal").unwrap(), &mut vm, &alice_auth)?;
    let proposal_id = find_proposal_id(&vm, &alice_auth)?;
    ...
    let edit_matches = cli_app.get_matches_from(vec![
        "proposal", "edit",
        "--id", &proposal_id,
        "--new-body", body_file_path.to_str().unwrap(),
    ...
    handle_proposal_command(edit_matches.subcommand_matches("proposal").unwrap(), &mut vm, &alice_auth)?;
    ...
    assert_eq!(proposal_v2.current_version, 2, "Version should have incremented after edit");
...
fn find_proposal_id(vm: &VM, auth_context: &AuthContext) -> Result<String, Box<dyn std::error::Error>> {
    let storage = vm.storage_backend.as_ref().ok_or("Storage missing")?;
    ...
    keys.iter()
        ...
        .max_by_key(|k| {
            ...
            k.split('/').nth(1).unwrap_or("").parse::<u64>().unwrap_or(0)
        })
...
fn publish_and_start_voting(vm: &mut VM, auth_context: &AuthContext, proposal_id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let cli_app = ProposalCli::command();
    let publish_matches = cli_app.get_matches_from(vec!["proposal", "publish", "--id", proposal_id]);
    handle_proposal_command(publish_matches.subcommand_matches("proposal").unwrap(), vm, auth_context)?;
    ...
    if prop.state != ProposalState::Voting {
        ...
        vm.storage_backend.as_mut().ok_or("Storage missing")?
            .set(Some(auth_context), "governance", &lifecycle_key, proposal_bytes)?;
    }
    Ok(())
} 