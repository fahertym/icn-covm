    ...
    S: StorageBackend + Send + Sync + Clone + Debug + 'static,
{
        ...
        F: FnOnce(&S, &AuthContext, &str) -> T,
    {
        ...
        let forked = self
            .fork()
            .expect("Failed to fork VM for storage operation");
...
fn setup_vm() -> VM<InMemoryStorage> {
    ...
    vm.execute(&[create_namespace_op])
    ...
    vm.execute(&[create_resources_op])
...
mod tests {
    ...
    fn test_create_resource() {
        ...
        assert!(
            balances_exists,
            "Resource balances were not created in storage"
        );
    }
    ...
    fn test_mint_and_burn() {
        ...
        assert!(
            burn_too_much.is_err(),
            "Burning more than available should fail"
        );
    }
    ...
    fn test_transfer() {
        ...
        let transfer_result =
            transfer_tokens(&mut vm, resource_id, alice, bob, transfer_amount, None);
        assert!(
        ...
        let transfer_too_much = transfer_tokens(&mut vm, resource_id, alice, bob, 100.0, None);
        assert!(
            transfer_too_much.is_err(),
    ...
    fn test_transfer_with_reason() {
        ...
        assert!(
            transfer_result.is_ok(),
            "Failed to transfer tokens with reason: {:?}",
            transfer_result
        );
    ...
    fn test_cross_resource_transfer() {
        ...
        assert_eq!(
            alice_silver,
            silver_amount - silver_transfer,
            "Alice's silver balance after transfer is incorrect"
        );
        assert_eq!(
            bob_silver, silver_transfer,
