...
fn setup_vm() -> VM {
    ...
    storage
        .create_account(Some(&auth_context), "test_user", 1_000_000)
        .unwrap();

    // Create the test namespace
    storage
        .create_namespace(Some(&auth_context), "test_namespace", 1_000_000, None)
        .unwrap();
...
fn test_create_resource() {
    ...
    let resource_exists = storage
        .contains(auth.as_ref(), &ns, "resources/test_coin")
        .unwrap();
    assert!(resource_exists, "Resource not found in storage");

    // Check that balances were initialized
    let balances_exists = storage
        .contains(auth.as_ref(), &ns, "resources/test_coin/balances")
        .unwrap();
...
fn test_mint_and_burn() {
    ...
    let mint_result = mint_tokens(
        ...
        "alice",
        50.0,
        Some("Bonus reward".to_string()),
    );
    assert!(
        mint_result.is_ok(),
        "Failed to mint additional tokens: {:?}",
        mint_result
    );
    ...
    assert!(
        burn_result.is_err(),
        "Should fail when burning more than available"
    );
}
...
fn test_transfer() {
    ...
    assert!(
        mint_result.is_ok(),
        "Failed to mint tokens to alice: {:?}",
        mint_result
    );
    ...
    assert_eq!(
        alice_balance, 60.0,
        "Alice's balance incorrect after transfer"
    );
    ...
    assert!(
        ...
        "Should fail when transferring more than available"
    );
}
...
fn test_integration_economic_ops() {
    ...
    assert!(
        result.is_ok(),
        "Failed to create community_token: {:?}",
        result
    );
    ...
    assert!(
        transfer_result.is_ok(),
        "Failed to fund treasury from founder2: {:?}",
        transfer_result
    );
    ...
    let transfer_result = transfer_tokens(
        ...
        "team",
        450.0,
        Some("Team allocation".to_string()),
    );
    assert!(
        transfer_result.is_ok(),
        "Failed to fund team: {:?}",
...
fn test_multiple_resources() {
    ...
    assert!(
        mint_result.is_ok(),
        "Failed to mint community_token: {:?}",
        mint_result
    );
    ...
    assert_eq!(
        community_token_balance, 200.0,
        "community_token balance incorrect"
    );
    ...
    assert!(
        transfer_result.is_ok(),
        "Failed to transfer community_token: {:?}",
        transfer_result
    );
    ...
    assert_eq!(
        alice_test_coin, 60.0,
        "Alice's test_coin balance incorrect after transfer"
    );

    let bob_test_coin = get_balance(&mut vm, "test_coin", "bob").unwrap();
    assert_eq!(
        bob_test_coin, 40.0,
        "Bob's test_coin balance incorrect after transfer"
    );

    let alice_community_token = get_balance(&mut vm, "community_token", "alice").unwrap();
    assert_eq!(
        alice_community_token, 150.0,
        "Alice's community_token balance incorrect after transfer"
    );

    let bob_community_token = get_balance(&mut vm, "community_token", "bob").unwrap();
    assert_eq!(
        bob_community_token, 50.0,
        "Bob's community_token balance incorrect after transfer"
    );
}
