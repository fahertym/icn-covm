...
fn test_json_authed_methods() {
    ...
    storage
        .create_namespace(Some(&auth), "test_namespace", 1_000_000, None)
        .unwrap();
    ...
    storage
        .set_json_authed(&auth, "test_namespace", "test_object", &test_data)
        .unwrap();

    // Test get_json_authed
    let retrieved: TestData = storage
        .get_json_authed(&auth, "test_namespace", "test_object")
        .unwrap();
    ...
    assert!(storage
        .contains_authed(&auth, "test_namespace", "test_object",)
        .unwrap());

    // Test list_keys_authed
    let keys = storage
        .list_keys_authed(&auth, "test_namespace", None)
        .unwrap();
    ...
    // Test delete_authed
    storage
        .delete_authed(&auth, "test_namespace", "test_object")
        .unwrap();
