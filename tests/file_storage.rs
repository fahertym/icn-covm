...
fn test_file_storage_namespaces() -> StorageResult<()> {
    ...
    storage.set(Some(&admin), "governance", "config", to_bytes("Global config"))?;
    storage.set(Some(&admin), "governance/proposals", "prop-001", to_bytes("Proposal 1"))?;
    storage.set(Some(&admin), "governance/votes", "prop-001/admin", to_bytes("1"))?;
    
    // Verify data in each namespace
    assert_eq!(from_bytes(&storage.get(Some(&admin), "governance", "config")?), "Global config");
    assert_eq!(from_bytes(&storage.get(Some(&admin), "governance/proposals", "prop-001")?), "Proposal 1");
    assert_eq!(from_bytes(&storage.get(Some(&admin), "governance/votes", "prop-001/admin")?), "1");
...
fn test_file_storage_transactions() -> StorageResult<()> {
    ...
    storage.set(Some(&admin), "test", "key1", to_bytes("Updated in transaction"))?;
    storage.set(Some(&admin), "test", "key2", to_bytes("New key in transaction"))?;
    
    // Values should be visible during the transaction
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key1")?), "Updated in transaction");
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key2")?), "New key in transaction");
    
    // Rollback the transaction
    storage.rollback_transaction()?;
    
    // Values should be restored to pre-transaction state
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key1")?), "Initial value");
    ...
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key1")?), "Final value");
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key2")?), "Another new key");
...
fn test_file_storage_permissions() -> StorageResult<()> {
    ...
    assert_eq!(from_bytes(&storage.get(Some(&reader), "test", "key1")?), "Test value");
    
    // Reader should not be able to write
    let write_result = storage.set(Some(&reader), "test", "key1", to_bytes("Modified by reader"));
    assert!(write_result.is_err());
    
    // Writer should be able to read and write
    assert_eq!(from_bytes(&storage.get(Some(&writer), "test", "key1")?), "Test value");
    storage.set(Some(&writer), "test", "key1", to_bytes("Modified by writer"))?;
    assert_eq!(from_bytes(&storage.get(Some(&writer), "test", "key1")?), "Modified by writer");
