...
fn get_test_dir() -> PathBuf {
    let test_dir = PathBuf::from("target/test/file_storage");
...
fn test_file_storage_basic() -> StorageResult<()> {
    ...
    // Set a value
    storage.set(&admin, "test", "key1", to_bytes("Hello, world!"))?;
    
    // Get the value back
    let value = storage.get(&admin, "test", "key1")?;
    assert_eq!(from_bytes(&value), "Hello, world!");
    
    // Update the value
    storage.set(&admin, "test", "key1", to_bytes("Updated value"))?;
    let value = storage.get(&admin, "test", "key1")?;
    assert_eq!(from_bytes(&value), "Updated value");
    
    // List versions
    let versions = storage.list_versions(&admin, "test", "key1")?;
    assert_eq!(versions.len(), 2);
    assert_eq!(versions[0].version, 1);
    assert_eq!(versions[1].version, 2);
    
    // Get specific version
    let (v1_data, v1_info) = storage.get_version(&admin, "test", "key1", 1)?;
...
fn test_file_storage_namespaces() -> StorageResult<()> {
    ...
    // List namespaces
    let namespaces = storage.list_namespaces(&admin, "governance")?;
    assert_eq!(namespaces.len(), 2);
    
    // Store data in different namespaces
    storage.set(&admin, "governance", "config", to_bytes("Global config"))?;
    storage.set(&admin, "governance/proposals", "prop-001", to_bytes("Proposal 1"))?;
    storage.set(&admin, "governance/votes", "prop-001/admin", to_bytes("1"))?;
    
    // Verify data in each namespace
    assert_eq!(from_bytes(&storage.get(&admin, "governance", "config")?), "Global config");
    assert_eq!(from_bytes(&storage.get(&admin, "governance/proposals", "prop-001")?), "Proposal 1");
    assert_eq!(from_bytes(&storage.get(&admin, "governance/votes", "prop-001/admin")?), "1");
...
fn test_file_storage_transactions() -> StorageResult<()> {
    ...
    // Initial value
    storage.set(&admin, "test", "key1", to_bytes("Initial value"))?;
    
    // Begin transaction
    storage.begin_transaction()?;
    
    // Make some changes in the transaction
    storage.set(&admin, "test", "key1", to_bytes("Updated in transaction"))?;
    storage.set(&admin, "test", "key2", to_bytes("New key in transaction"))?;
    
    // Values should be visible during the transaction
    assert_eq!(from_bytes(&storage.get(&admin, "test", "key1")?), "Updated in transaction");
    assert_eq!(from_bytes(&storage.get(&admin, "test", "key2")?), "New key in transaction");
    
    // Rollback the transaction
    storage.rollback_transaction()?;
    
    // Values should be restored to pre-transaction state
    assert_eq!(from_bytes(&storage.get(&admin, "test", "key1")?), "Initial value");
    ...
    // Make changes again
    storage.set(&admin, "test", "key1", to_bytes("Final value"))?;
    storage.set(&admin, "test", "key2", to_bytes("Another new key"))?;
    
    // Commit the transaction
    storage.commit_transaction()?;
    
    // Values should persist after commit
    assert_eq!(from_bytes(&storage.get(&admin, "test", "key1")?), "Final value");
    assert_eq!(from_bytes(&storage.get(&admin, "test", "key2")?), "Another new key");
...
fn test_file_storage_permissions() -> StorageResult<()> {
    ...
    storage.create_account(&admin, "admin_user", 1024*1024)?;
    storage.create_account(&admin, "reader_user", 1024*1024)?;
    storage.create_account(&admin, "writer_user", 1024*1024)?;
    storage.create_namespace(&admin, "test", 1024*1024, None)?;
    
    // Set initial value as admin
    storage.set(&admin, "test", "key1", to_bytes("Test value"))?;
    ...
    assert_eq!(from_bytes(&storage.get(&reader, "test", "key1")?), "Test value");
    
    // Reader should not be able to write
    let write_result = storage.set(&reader, "test", "key1", to_bytes("Modified by reader"));
    assert!(write_result.is_err());
    
    // Writer should be able to read and write
    assert_eq!(from_bytes(&storage.get(&writer, "test", "key1")?), "Test value");
    storage.set(&writer, "test", "key1", to_bytes("Modified by writer"))?;
    assert_eq!(from_bytes(&storage.get(&writer, "test", "key1")?), "Modified by writer");
