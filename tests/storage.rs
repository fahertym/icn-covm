...
fn test_in_memory_storage() -> StorageResult<()> {
    ...
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key1")?), "42.0");
    
    storage.set(Some(&admin), "test", "key2", to_bytes("123.45"))?;
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key2")?), "123.45");
...
fn test_in_memory_storage_transaction() -> StorageResult<()> {
    ...
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key1")?), "20.0");
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key2")?), "30.0");
    
    // Rollback transaction
    storage.rollback_transaction()?;
    
    // Values should be restored to pre-transaction state
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key1")?), "10.0");
    ...
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key1")?), "50.0");
    assert_eq!(from_bytes(&storage.get(Some(&admin), "test", "key2")?), "60.0");
